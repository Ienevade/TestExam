...
def main():
    ...
    return render_template('old_main.html', spirit=False, his1=history1, his2=history2, numhis=session['numhis'],
                           ran=len(history2), message='Теперь введите двузначное число', firtrue=session['firtrue'],
...
def ready():
    ...
    return render_template('old_main.html', spirit=True, nums=nums, his1=history1, his2=history2, numhis=session['numhis'],
...
def commit():
    ...
    if session['flag_ready'] == True and request.form['Number'] and len(request.form['Number']) == 2 and request.form['Number'] >0 and string.digits:
        if 'history1' not in session:
            session['history1'] = []
            session['history2'] = []
